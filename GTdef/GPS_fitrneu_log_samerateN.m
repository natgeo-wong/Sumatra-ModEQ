function [ gparam,err,wrms,rchi2 ] = GPS_fitrneu_log_samerateN(rneuCell,dflagCell,Teq,dday)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                        GPS_fitrneu_log_samerateN.m				%
...
%										%
% uses GPS_lsqnonlin_logfunc_samerateN.m					%
% [ diff ] = GPS_lsqnonlin_logfunc_samerateN(xx,rneu,Teq)			%
%   xx = [ tau n_b n_v n_O n_a      						%
%              e_b e_v e_O e_a      						%
%              u_b u_v u_O u_a ]						%
% first created by lfeng Wed Mar 28 11:23:34 SGT 2012				%
% last modified by lfeng Wed Mar 28 11:37:04 SGT 2012				%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lsqnonlin(@(xx) GPS_lsqnonlin_logfunc_samerateN(xx,rneuCurrCell,Teq),x0,lb,ub,options);	
% check if successful
%if exitflag~=1, error('GPS_fitrneu_log_samerateN ERROR: do not converge!'); end
% how good is the fit!
chi2  = resnorm;
...
gparam = zeros(staNum,fparNum);
gparam(:,1:4)       = oparam(:,1:4);
gparam(:,6:9)       = oparam(:,5:8);
gparam(:,11:14)     = oparam(:,9:12);
gparam(:,[5 10 15]) = tau;
