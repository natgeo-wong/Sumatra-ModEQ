function [ gamra ] = GAMRA_readhdf5_summary(fileName)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                          GAMRA_readhdf5_summary                               %
% read GAMRA output summary files                                               %
...
if ~exist(fileName,'file'), error('GAMRA_readhdf5_summary ERROR: %s does not exist!',fileName); end
...
% ---------------------- Basic Info ----------------------
gamra.dim           = h5read(fileName,'/BASIC_INFO/number_dimensions_of_problem'); % scalar =3
gamra.varNum        = h5read(fileName,'/BASIC_INFO/number_visit_variables'); % scalar =5
gamra.varName       = h5read(fileName,'/BASIC_INFO/var_names'); % [varNum*1] cell
gamra.compNum       = h5read(fileName,'/BASIC_INFO/var_number_components'); % [varNum*1] vector
gamra.patchNum      = h5read(fileName,'/BASIC_INFO/number_global_patches'); % scalar
gamra.levelNum      = h5read(fileName,'/BASIC_INFO/number_levels'); % scalar
gamra.levelpatchNum = h5read(fileName,'/BASIC_INFO/number_patches_at_level'); % [levelNum*1] vector, should sum up to patchNum
gamra.ratio         = h5read(fileName,'/BASIC_INFO/ratios_to_coarser_levels'); % [dim*levelNum] matrix, all -1 for the 1st level
gamra.scaling       = h5read(fileName,'/BASIC_INFO/scaling'); % [varNum*1] vector

% ---------------------- Parent Topo ----------------------
gamra.parentNum     = h5read(fileName,'/BASIC_INFO/parent_array_length'); % scalar
if gamra.parentNum>1
   gamra.parent     = h5read(fileName,'/BASIC_INFO/parent_array'); % [parentNum*1] vector, index in the patch list
   gamra.parentPnter= h5read(fileName,'/BASIC_INFO/parent_pointer_array'); % struct (offset,number_parents [patchNum*1])
else
   gamra.parent = []; gamra.parentPnter = [];
end

% ---------------------- Child Topo ----------------------
gamra.childNum      = h5read(fileName,'/BASIC_INFO/child_array_length'); % scalar
if gamra.parentNum>1
   gamra.child      = h5read(fileName,'/BASIC_INFO/child_array'); % [childNum*1] vector, index in the patch list
   gamra.childPnter = h5read(fileName,'/BASIC_INFO/child_pointer_array');; % struct (offset,number_children [patchNum*1])
else
   gamra.child = []; gamra.childPnter = [];
end

% ---------------------- Patch ----------------------
gamra.dx            = h5read(fileName,'/BASIC_INFO/dx'); % [dim*levelNum] matrix
gamra.patch_extents = h5read(fileName,'/extents/patch_extents'); % struct (lower,upper,xlo,xup [dim*patchNum])
gamra.patch_map     = h5read(fileName,'/extents/patch_map'); % struct (processor_number,file_cluster_number,level_number,patch_number [patchNum*1] int32)
