function [ rneuPost,rneuMegaPost ] = GPS_lsqnonlin_1site_postEQs(rneu,eqMat,funcMat,fqMat,xx,tt0)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                       GPS_lsqnonlin_1site_postEQs                           %
% keep postseismic deformation of all EQs                                     %
% remove background rates, seasonal signals & all offsets                     %
% from the time series according to the values of xx                          %
%                                                                             %
% INPUT:                                                                      %
...
%                                                                             %
% OUTPUT:                                                                     %
% rneuPost - data with rates, seasonal signals & all offsets removed          %
%            errors are kept the same                                         %
%                                                                             %
% first created by Lujia Feng Sat Oct 27 12:53:18 SGT 2012                    %
% modified funcMat lfeng Thu Apr  4 16:17:01 SGT 2013                         %
% added time reference tt0 lfeng Wed Jan  8 12:58:10 SGT 2014                 %
% added lg2 exp2 & corrected regexpi lfeng Wed Mar  5 11:21:08 SGT 2014       %
...
% added rneuMegaPost for postseismic only on megathrust lfeng Thu Apr  9 2015 %
% last modified by Lujia Feng Thu Apr  9 16:00:10 SGT 2015                    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for jj=1:eqNum
    ...
    if  ~isempty(regexpi(func,'(log|lng|lg2|ln2)'))
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               if strfind(funcN,'ln');
                  nnMega(ind2) = nnMega(ind2)-nnaa*log(1+(tt(ind2)-Teq)/nntau); 
               else
                  nnMega(ind2) = nnMega(ind2)-nnaa*log10(1+(tt(ind2)-Teq)/nntau); 
               end
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               if strfind(funcE,'ln');
                  eeMega(ind2) = eeMega(ind2)-eeaa*log(1+(tt(ind2)-Teq)/eetau); 
               else
                  eeMega(ind2) = eeMega(ind2)-eeaa*log10(1+(tt(ind2)-Teq)/eetau); 
               end
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               if strfind(funcU,'ln');
                  uuMega(ind2) = uuMega(ind2)-uuaa*log(1+(tt(ind2)-Teq)/uutau); 
               else
                  uuMega(ind2) = uuMega(ind2)-uuaa*log10(1+(tt(ind2)-Teq)/uutau); 
               end
            end
        end
    end
    ...
    if ~isempty(regexpi(func,'(exp|ep2)'))
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               nnMega(ind2) = nnMega(ind2)-nnaa*(1-exp((Teq-tt(ind2))/nntau));
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               eeMega(ind2) = eeMega(ind2)-eeaa*(1-exp((Teq-tt(ind2))/eetau));
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               uuMega(ind2) = uuMega(ind2)-uuaa*(1-exp((Teq-tt(ind2))/uutau));
            end
        end
    end
    ...
    if regexpi(func,'k[0-9]{2}_\w+')
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               nnMega(ind2) = nnMega(ind2)-nnaa*(1-2/kk*acoth(coth(0.5*kk)*exp((tt(ind2)-Teq)/nntau)));
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               eeMega(ind2) = eeMega(ind2)-eeaa*(1-2/kk*acoth(coth(0.5*kk)*exp((tt(ind2)-Teq)/eetau)));
            end
        end
	    ...
	    % remove non-megathrust EQs
            if eqMat(jj,1) == 20120411 || eqMat(jj,1) == 20050410 
               uuMega(ind2) = uuMega(ind2)-uuaa*(1-2/kk*acoth(coth(0.5*kk)*exp((tt(ind2)-Teq)/uutau)));
            end
        end
    end
end
...
rneuPost = rneu;
rneuPost(:,3:5) = [ nn ee uu ];
rneuMegaPost = rneu;
rneuMegaPost(:,3:5) = [ nnMega eeMega uuMega ];

% if on EQs are non-megathrust
if isequal(rneuPost,rneuMegaPost)
   rneuMegaPost = [];
end
