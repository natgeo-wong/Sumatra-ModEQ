function [ plo,pup,plevel,clo,cup,clevel,lambda,mu,disp,strain ] = GAMRA_locate_pnts(gamra,gamout,xpnt)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                             GAMRA_locate_pnts                                 %
% find values for points in GAMRA database gamra & gamout                       %
%                                                                               %
% INPUT:                                                                        %
...
% data
xlo    = gamra.patch_extents.xlo;
xup    = gamra.patch_extents.xup;
level  = gamra.patch_map.level_number;
% loop through points
for ii=1:pntNum
   % find patch that constains point
   xx      = xpnt(:,ii); 
   pInd    = GAMRA_check_inside(xlo,xup,xx);
   inlev   = level(pInd);
   pInd    = pInd(inlev==max(inlev));
   if isempty(pInd)
      error('GAMRA_locate_pnts ERROR: [%f %f %f] is outside the model domain!',xx);
   end
   if length(pInd)>1
      error('GAMRA_locate_pnts ERROR: [%f %f %f] belongs to >1 patches at the highest level!',xx);
   end
   plo(:,ii)    = xlo(:,pInd);
   pup(:,ii)    = xup(:,pInd);
   ...
   cInd    = GAMRA_check_inside(xloCell,xupCell,xx);
   % assign values
   clo(:,ii)    = xloCell(:,cInd);
   cup(:,ii)    = xupCell(:,cInd);
...
function [ inInd ] = GAMRA_check_inside(xlo,xup,xx)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                            GAMRA_check_inside                                 %
% check if points reside inside extents                                         %
%                                                                               %
% INPUT:                                                                        %
