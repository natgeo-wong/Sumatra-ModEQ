function [ prf ] = grdprofile(fgrdName,fprfName,method,prfFormat,pars)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                grdprofile.m				  	%
% create profile transects and get corresponding values from grd file           %
% Note: need to install GMT Matlab mex for grdread 				%
%									  	%
% INPUT:					  		  	  	%
% fgrdName - GMT grd data file						  	%
% fprfName - output profile file					  	%
%   fprfName = '' do not write an output file			  		%
% method  - interpolation method for Matlab function interp2			%
...
%		       depth for EQs					  	%
%										%
% based on xyzprofile.m lfeng Fri Oct 14 21:18:08 SGT 2011			%
% first created by lfeng Fri Oct 14 21:18:20 SGT 2011				%
% last modified by lfeng Fri Oct 14 23:41:36 SGT 2011				%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch prfFormat
   ...
   otherwise
      error('grdprofile ERROR: profile input parameters are wrong!');
end

%%%%%%%%%% read in grd file %%%%%%%%%%
[ grdLon,grdLat,meshZZ ] = grdread(fgrdName);			% grdLon & grdLat are vectors
[ meshLon,meshLat ] = meshgrid(grdLon,grdLat);
[ meshXX,meshYY ] = LL2ckmd(meshLon,meshLat,lon0,lat0,0);

%%%%%%%%%% interpolate values along profile %%%%%%%%%%
% interpolate in a local cartesian coordinate instead of latlon space
% prfXX - row vector; prfYY - column vector
prfXX = sort(prf.xx); prfYY = sort(prf.yy)';			% order ascending in case the profile is some random
prfmeshZZ = interp2(meshXX,meshYY,meshZZ,prfXX,prfYY,method);

%%%%%%%%%% calculate distance & find the interpolated value %%%%%%%%%%
for ii=1:num
   ...
   indXX = find(prfXX==xx);
   indYY = find(prfYY==yy);
   prf.zz(ii) = prfmeshZZ(indYY,indXX);				% switch xx & yy: xx for columns; yy for rows
end
